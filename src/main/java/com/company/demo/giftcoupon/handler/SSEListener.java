...
public class SSEListener implements CouponEventHandler {
    ...
    private void pushSSE (DomainEventEnvelope<CouponIssuedPayload> envelope) throws IOException {
        ...
        SseEmitter emitter = sseEmitterRepository.findById(payload.memberId());
        ...
        if (emitter != null) {
            try {
                ...
                log.info("SSE 데이터 전송 성공 - UserId: {}", payload.memberId());
            } catch (Exception e) {
                log.error("SSE 데이터 전송 실패 - UserId: {}", payload.memberId(), e.getMessage(), e);
                sseEmitterRepository.deleteById(payload.memberId());
