...
public class StatisticsRepository {
    ...
    public void incrementIssuedCount(Long couponId) {
        String key = redisUtil.getIssuedCountKey(couponId);
        redisUtil.incrementValue(key, 1);
    }

    public void incrementEstimatedRevenue(Long couponId, Long couponValue) {
        String key = redisUtil.getEstimatedRevenueKey(couponId);
        redisUtil.incrementValue(key, couponValue);
    }

    public void incrementGenderCount(Long couponId, String gender) {
        String key = redisUtil.getGenderCountsKey(couponId);
        redisUtil.incrementHashField(key, gender, 1);
    }

    public void incrementAgeGroupCount(Long couponId, String ageGroup) {
        String key = redisUtil.getAgeGroupCountsKey(couponId);
    ...
    public Long getEstimatedRevenue(Long couponId) {
        String key = redisUtil.getEstimatedRevenueKey(couponId);
        Object value = redisUtil.getValue(key);
        return value == null ? 0L : Long.valueOf(value.toString());
    }

    public Map<String, Long> getGenderCounts(Long couponId) {
        String key = redisUtil.getGenderCountsKey(couponId);
        return redisUtil.getHashEntries(key);
    }

    public Map<String, Long> getAgeGroupCounts(Long couponId) {
        String key = redisUtil.getAgeGroupCountsKey(couponId);
        return redisUtil.getHashEntries(key);
    }
